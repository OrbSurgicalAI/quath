...
use crate::{
    ClientProtocolError, ClientRevoke, DsaSystem, HashingAlgorithm, KemAlgorithm,
    ServerErrorResponse, ServerRevoke, protocol::ProtocolKit,
};
    ...
    H: HashingAlgorithm<N>,
{
    ...
    match packet {
        ClientRevokeInput::Response(response) => {
            ProtocolKit::<S, K, H, N>::client_revoke_finish(
                &response,
                &inner.token_hash,
                sig,
                &inner.server_pk,
        ...
        ClientRevokeInput::ErrorResponse(error) => {
...
mod tests {
    ...
    pub fn test_client_revoke_happy() {
        ...
        let server_response =
            ProtocolKit::<FauxChain, FauxKem, Sha3_256, 32>::server_revoke(&request, &c_pk, &s_sk)
                .unwrap();
    ...
    fn test_client_revoke_receives_error_response() {
        ...
        driver.recv(Some(super::ClientRevokeInput::ErrorResponse(
            error_response,
        )));
