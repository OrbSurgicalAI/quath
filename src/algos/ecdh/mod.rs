use k256::{ecdh, elliptic_curve::{self, ecdh::{EphemeralSecret, SharedSecret}, rand_core::OsRng, sec1::FromEncodedPoint}, EncodedPoint, PublicKey, Secp256k1};
...
impl KEMAlgorithm for K256ECDH {
    ...
    fn encapsulate(encap_key: &Self::EncapsulationKey, _: &Self::Context) -> Result<(Self::CipherText, Self::SharedSecret), Self::Error> {
        ...
        let shared_secret = bob_secret.diffie_hellman(&PublicKey::from_encoded_point(encap_key).unwrap());
    ...
    fn decapsulate(decap_key: &Self::DecapsulationKey, cipher: &Self::CipherText, context: &Self::Context) -> Result<Self::SharedSecret, Self::Error> {
        let shared_secret = decap_key.diffie_hellman(&PublicKey::from_encoded_point(cipher).unwrap());
        Ok(shared_secret)
    }
