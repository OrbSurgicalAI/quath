    ...
    S: DsaSystem,
{
    ...
    pub verify_register_integrity:
        fn(VerifyRequestIntegrityQuery<S>) -> VerifyRequestIntegrityResponse<S>,
    ...
    H: HashingAlgorithm<N>,
{
    pub fn register(
        ...
        while poll.is_pending() {
            if let Some(transmit) = machine.poll_transmit() {
                match transmit {
                    ServerRegistryOutput::StoreRegistry(query) => machine.recv(
                        time,
                        Some(ServerRegistryInput::StoreResponse((self.store_registry)(
                            query,
                        ))),
                    ),
    ...
    pub fn deregister(
        ...
        machine.recv(Some(crate::ServerDeregisterInput::Request(request)));
            ...
            if let Some(transmit) = machine.poll_transmit() {
                match transmit {
                    ServerDeregisterOutput::GetPublicKey(query) => {
                        machine.recv(Some(ServerDeregisterInput::KeyFetchResponse((self
                            .deregister_public_key_fetch)(
                            query
                        ))))
                    }
                    ServerDeregisterOutput::Deregister(query) => {
                        machine.recv(Some(ServerDeregisterInput::DeregisterResponse((self
                            .deregister_entity)(
                            query
                        ))));
                    }
            ...
            if let Some(transmit) = machine.poll_transmit() {
                match transmit {
                    ...
                    ServerTokenOutput::StorageRequest(query) => {
                        machine.recv(
                            time,
                            Some(ServerTokenInput::StorageResponse((self.store_token)(query))),
                        );
                    }
                    ServerTokenOutput::VerificationRequest(query) => {
                        machine.recv(
                            time,
                            Some(ServerTokenInput::VerifyResponse((self.verify_token)(query))),
                        );
                    }
                }
            }
    ...
    pub fn cycle(
        ...
        machine.recv(time, Some(crate::ServerCycleInput::ReceiveRequest(request)));
        ...
        while poll.is_pending() {
            if let Some(transmit) = machine.poll_transmit() {
                match transmit {
                    ServerCycleOutput::VerificationRequest(query) => machine.recv(
                        time,
                        Some(crate::ServerCycleInput::VerificationResponse((self
                            .cycle_verify)(
                            query
                        ))),
                    ),
                    ServerCycleOutput::StorageRequest(query) => machine.recv(
                        time,
                        Some(crate::ServerCycleInput::StoreResponse((self
                            .store_registry)(
                            query
                        ))),
                    ),
