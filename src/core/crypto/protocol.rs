...
pub type ServerTokenResult<const H: usize, K, S> =
    ServerProtocolResult<(ServerToken<H, K, S>, Token<Final>)>;
...
pub type ClientRegisterResult<PUB, SIG, PRIV> =
    ClientProtocolResult<(ClientRegisterInit<PUB, SIG>, PRIV)>;
    ...
    H: HashingAlgorithm<HS>,
{
    ...
    pub fn client_register_init(
        ...
        let (public, private) =
            S::generate().map_err(|_| ClientProtocolError::FailedToGenerateDsaPair)?;
    ...
    pub fn client_cycle_init(
        ...
        let new_proof = new_private
            .sign_bytes(&body.to_bytes())
            .map_err(|_| ClientProtocolError::FailedToSignRequest)?;

        // The proof from the old key.
        let original_proof = old_private
            .sign_bytes(&new_proof.view())
            .map_err(|_| ClientProtocolError::FailedToSignRequest)?;
    ...
    pub fn server_cycle(
        ...
        if !in_msg
            .body
            .new_public_key
            .verify(&in_msg.body.to_bytes(), &in_msg.new_proof)
        {
        ...
        let body = ServerCycleBody {
            code: OpCode::CycleOk,
            hash: B64(H::hash_sequence(&[
                (&in_msg.body.identifier.to_bytes_le()) as &[u8],
                &in_msg.body.new_public_key.view(),
            ])),
        };
        ...
        F: FnMut(&mut Token<Pending>),
    {
        ...
        let (dk, ek) = K::generate().map_err(|_| ClientProtocolError::FailedToGenerateKemPair)?;
    ...
    pub fn server_token(
        ...
        let (cipher_text, shared_secret) =
            K::encapsulate(&body.ek).map_err(|_| ServerProtocolError::EncapsulationFailed)?;
    ...
    ) -> ClientProtocolResult<ClientDeregister<S::Signature, HS>> {
        let combined = H::hash_sequence(&[&target.to_bytes_le(), &claimaint_id.to_bytes_le()]);
        let signature = claimant_sk
            .sign_bytes(&combined)
            .map_err(|_| ClientProtocolError::FailedToSignRequest)?;
    ...
    ) -> ClientProtocolResult<()>
    {
        ...
        if !server_pk.verify(&*response.approval, &*response.proof) {
            return Err(ClientProtocolError::FailedToAuthenticateDeregisterResponse);
        }
...
mod tests {
    ...
    fn execute_protocol_run() -> anyhow::Result<(), String> {
        ...
        let (req, client_private) =
            ProtocolKit::<MlDsa44, MlKem512, Sha3_256, 32>::client_register_init(
                client_id,
                admin_id,
                &admin_private,
            )
        ...
        let server_registry =
            QuantumKitL1::server_register(&req, &admin_public, &server_private).unwrap();

        // Client finishes registry
        QuantumKitL1::client_register_finish(&server_registry, client_id, &server_public).unwrap();

        // client response
        let (cycle_init, client_private) =
            QuantumKitL1::client_cycle_init(client_id, &client_private).unwrap();

        let new_public = cycle_init.body.new_public_key.clone();

        // server answers cycle.
        let serv = ProtocolKit::<MlDsa44, MlKem512, Sha3_256, 32>::server_cycle(
            &cycle_init,
            &req.body.public_key,
            &server_private,
        )
        ...
        let (res, server_final_token) = QuantumKitL1::server_token(
            &req,
            &new_public,
            &server_private,
        ...
        let client_final_token =
            QuantumKitL1::client_token_finish(&res, &req.body.token, &decapskey, &server_public)
                .unwrap();
    ...
    #[test]
    pub fn test_deregister_run() -> anyhow::Result<(), &'static str> {
        let claimant_id = Uuid::new_v4();
        let (claim_pk, claim_sk) = FauxChain::generate().unwrap();
        let (server_pk, server_sk) = FauxChain::generate().unwrap();
