...
mod testing {
    ...
    use super::super::{
        DsaSystem, FixedByteRepr, KemAlgorithm, Parse, PrivateKey, PublicKey, Signature, ViewBytes,
    };
        ...
        fn view(&self) -> Cow<'_, [u8]> {
            Cow::Owned(
                self.id
                    .to_le_bytes()
                    .iter()
                    .chain(self.private.to_le_bytes().iter())
                    .copied()
                    .collect(),
            )
        }
    ...
    impl KemAlgorithm for FauxKem {
        ...
        fn generate() -> Result<(Self::DecapsulationKey, Self::EncapsulationKey), Self::Error> {
            ...
            Ok((FauxDecapKey { secret }, FauxEncapKey { secret }))
        }
        ...
        fn decapsulate(
            ...
            Ok(FauxSharedSecret(
                decap_key.secret[0..32].try_into().unwrap(),
            ))
        }
    }
}
